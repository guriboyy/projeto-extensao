...
interface Atividade {
  id: number;
  title: string;
  tipo: string;   // <-- Campo que você está filtrando
  start: Date;
  end: Date;
}
...
function FiltroAtividades({ atividades, onSelecionarAtividades }: FiltroAtividadesProps) {
  const tiposAtividades = [...new Set(atividades.map(a => a.tipo))].filter(tipo => tipo !== '');
  ...
  if (tiposSelecionados.length === 0) {
    ...
    const eventosFiltrados = atividades.filter(a => tiposSelecionados.includes(a.tipo));
